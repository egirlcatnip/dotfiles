...
class azTaskbarAppIconIndicator extends St.DrawingArea {
    ...
    _setIndicatorColor(appState) {
        if (appState === Enums.AppState.RUNNING)
            this._indicatorColor = TaskbarManager.settings.get_string('indicator-color-running');
        else if (appState === Enums.AppState.FOCUSED)
            this._indicatorColor = TaskbarManager.settings.get_string('indicator-color-focused');
    }
